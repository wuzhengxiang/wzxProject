...
- (BOOL)validateAge {
    ...
    NSString * ageRegex = @"120|((1[0-1]|\\d)?\\d)";
    return [self isValidateByRegex:ageRegex];
}
...
- (BOOL)validateEmpty {
    ...
    NSString * empty = @".*[^ ].*";
    return [self isValidateByRegex:empty];
}
...
- (BOOL)isMobileNumber {
     ...
     */
    //    NSString *mobileNoRegex = @"^1((3\\d|5[0-35-9]|7[0-16-8]|8\\d)\\d|70[059])\\d{7}$";//除4以外的所有个位整数，不能使用[^4,\\d]匹配，这里是否iOS Bug?
    ...
    if ([self isEqualToString:@"00000000000"] || [self isEqualToString:@"000000000000"]) {
